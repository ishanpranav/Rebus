...
namespace Rebus.Server.ExecutionContexts
{
    internal sealed class UserExecutionContext : ExecutionContext
    {
        ...
        public UserExecutionContext(User user, Controller controller, RebusDbContext dbContext, IReadOnlyCollection<int> units, int commodity) : base(controller, dbContext, units, commodity)
        {
            _user = user;
        }
